...
//
// PING и HEALTH маршрутизируются до разбора команды предметной области —
// служебные команды транспортного уровня, у них нет command_id, они не идут
// в БД и не касаются книги (docs/task4/02-network-protocol.md, раздел 2,
// REQ-EXT-07). PRINT тоже не идёт в CommandProcessor: читает книгу напрямую
// через processor_.orderBook(), как и в консольном режиме — у него тоже нет
