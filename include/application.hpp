...
public:
    ...
    // batch (--batch) допустим только вместе с --replay — без
...
private:
    ...
    // сделок исходной записи здесь не нужно). Printed — команда PRINT, книга напечатана. Failed — ошибка
    ...
    //
    // batch — только для режима --replay --batch: сопоставление
    ...
    // Режим воспроизведения нагрузки (REQ-PERF-02): читает файл
    ...
    // (например путь указывает на каталог:
    ...
    //
    // batch включает пакетную запись (REQ-OPT-03): вместо
