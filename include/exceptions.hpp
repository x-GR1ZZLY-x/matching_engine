...
// Значение не проходит проверку предметной области (код
...
// Сбой сохранения результата команды в БД (REQ-TX-03). Наследует
...
// Ошибка клиентской стороны сетевого протокола (REQ-CLI-04):
...
// Отличать эти два случая по типу нужно там, где обрыв
