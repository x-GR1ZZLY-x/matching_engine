...
// matching-engine-client — библиотека одна, второго клиента,
...
public:
    ...
    //
    // После NetworkTimeoutError, брошенного отсюда, объект повторному
    // использованию не подлежит: обработчик async_resolve/async_connect,
    // не успевший отработать до таймаута, остаётся в очереди io_context_ и
    // выполнится при следующем run_for() как устаревший. Нужно создать
    // новый Client.
    ...
    // Ниже — примитивы для работы с байтами на проводе напрямую, а не
    // только целыми запросами. Их использует и matching-engine-client
    // (src/client_main.cpp): строка ввода уходит как есть, без разбора и
    // повторной пересборки JSON, поэтому convenience-метод request() выше,
    // принимающий nlohmann::json, ей не подходит. И тесты протокола
    // (фрагментация, склейка кадров, превышение размера) — им нужен тот же
    // прямой доступ к байтам. Кадр всё равно собирается кодеком, а не
    // вручную — манипуляции с байтами вне MessageCodec запрещены
    // (REQ-PROTO-05).
