...
SignalHandler::~SignalHandler() {
    ...
    if (rc != 0) {
        // Отказ pthread_kill (реалистично — ESRCH) означает, что будить уже
        // некого: поток вышел сам, до этого места, — например, sigwait()
        // получил внешний SIGUSR1 или завершился с ошибкой. join() ниже в
        // этом случае вернётся немедленно; строка в журнале нужна, чтобы
        // такой выход не остался незамеченным.
        Logger::instance().error(
